    ...
    <AdminPageShell title="Kasir POS">
      {step === "order" && (
        <div className="flex gap-4" style={{ minHeight: "calc(100vh - 140px)" }}>
          {/* LEFT */}
          <div
            className="w-64 shrink-0 flex flex-col gap-4 p-4 rounded-xl"
            style={{ background: "#0f0e00", border: "1px solid #D4AF3730" }}
          >
            <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {TREATMENTS.map(t => (
                <button
                  key={t.name}
                  onClick={() => addToCart(t)}
                  className="p-5 rounded-xl text-left transition-all hover:scale-[1.02] active:scale-95"
                  style={{ background: "linear-gradient(135deg,#1a1400,#0f0e00)", border: "1px solid #D4AF3750", cursor: "pointer" }}
                >
          ...
          <div className="w-72 shrink-0 flex flex-col rounded-xl" style={{ background: "#0f0e00", border: "1px solid #D4AF3730" }}>
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-2">
              {cart.length === 0 && (
                <p className="text-xs text-gray-600 text-center mt-8">Belum ada treatment dipilih</p>
              )}
              {cart.map((item, idx) => (
            ...
            <div className="p-4 flex flex-col gap-3" style={{ borderTop: "1px solid #D4AF3720" }}>
              ...
              <button
                ...
                style={{
                  background: cart.length > 0 ? "linear-gradient(135deg,#C9A84C,#f5e070,#B8960C)" : "#222",
                  color: cart.length > 0 ? "#080800" : "#444",
          ...
          <div className="w-full max-w-lg rounded-2xl p-8 flex flex-col gap-6" style={{ background: "#0f0e00", border: "1px solid #D4AF3750" }}>
            ...
            <div style={{ borderTop: "1px solid #D4AF3720", paddingTop: 16 }}>
              ...
              {cart.map((item, idx) => (
                <div key={idx} className="flex justify-between py-1">
                  <span className="text-sm text-gray-400">{item.name} ({item.duration} mnt)</span>
                  <span className="text-sm" style={{ color: "#e0c97f" }}>{formatRp(item.price)}</span>
                </div>
              ))}
              ...
              <div className="grid grid-cols-3 gap-3">
                {(["Cash", "QRIS", "Transfer"] as PayMethod[]).map(m => (
                  <button
                    ...
                    style={{
                      background: payMethod === m ? "linear-gradient(135deg,#C9A84C,#f5e070,#B8960C)" : "#111",
                      color: payMethod === m ? "#080800" : "#666",
                      border: payMethod === m ? "none" : "1px solid #333",
                      cursor: "pointer",
                    }}
                  >
                    {m}
            ...
            {payMethod === "Cash" && (
              <div>
                ...
                {cashReceived >= subtotal && subtotal > 0 && (
                  <p className="text-sm mt-2" style={{ color: "#4ade80" }}>Kembalian: {formatRp(change)}</p>
                )}
                {cashReceived > 0 && cashReceived < subtotal && (
                  <p className="text-sm mt-2 text-red-400">Kurang: {formatRp(subtotal - cashReceived)}</p>
                )}
              </div>
            )}
            ...
            <div className="flex gap-3">
              <button
                ...
                style={{ background: "#111", border: "1px solid #333", color: "#666", cursor: "pointer" }}
              >
                Batal
              </button>
              <button
                ...
                disabled={!canPay}
                className="flex-1 py-3 rounded-lg font-bold text-sm tracking-wider"
                style={{
                  background: canPay ? "linear-gradient(135deg,#C9A84C,#f5e070,#B8960C)" : "#222",
                  color: canPay ? "#080800" : "#444",
          ...
          <div className="w-full max-w-md rounded-2xl p-8 flex flex-col gap-4" style={{ background: "#0f0e00", border: "1px solid #D4AF3750" }}>
            ...
            <div style={{ borderTop: "1px solid #D4AF3720", paddingTop: 16 }} className="flex flex-col gap-2">
              {[
                ...
                ["Terapis", invoice.therapist],
                ["Kamar", invoice.room],
              ].map(([k, v]) => (
                <div key={k} className="flex justify-between">
                  <span className="text-xs text-gray-500">{k}</span>
                  <span className="text-sm" style={{ color: "#e0c97f" }}>{v}</span>
                </div>
              ))}
            </div>
            ...
            <div style={{ borderTop: "1px solid #D4AF3720", paddingTop: 16 }} className="flex flex-col gap-2">
              <div className="flex justify-between">
                <span className="text-sm font-bold" style={{ color: "#f5e070" }}>Total</span>
                <span className="text-xl font-bold" style={{ color: "#f5e070" }}>{formatRp(invoice.subtotal)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-xs text-gray-500">Metode</span>
                <span className="text-sm" style={{ color: "#C9A84C" }}>{invoice.payMethod}</span>
              </div>
              {invoice.payMethod === "Cash" && (
                <>
                  <div className="flex justify-between">
                    <span className="text-xs text-gray-500">Tunai Diterima</span>
                    <span className="text-sm" style={{ color: "#e0c97f" }}>{formatRp(invoice.cashReceived)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-xs text-gray-500">Kembalian</span>
                    <span className="text-sm" style={{ color: "#4ade80" }}>{formatRp(invoice.cashReceived - invoice.subtotal)}</span>
                  </div>
                </>
              )}
              <div className="flex justify-between">
                <span className="text-xs text-gray-500">Komisi Terapis (35%)</span>
                <span className="text-sm" style={{ color: "#C9A84C" }}>{formatRp(invoice.commission)}</span>
              </div>
            </div>

            <button
              onClick={resetTransaction}
              className="w-full py-3 rounded-lg font-bold text-sm tracking-wider mt-2"
